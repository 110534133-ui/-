<?php
header('Content-Type: application/json; charset=utf-8');
session_start();
include 'config.php';

if (!isset($_SESSION['member_phone'])) {
    echo json_encode(['success' => false, 'message' => '尚未登入，請先登入帳號']);
    exit;
}
$phone = $_SESSION['member_phone'];

if (!isset($_POST['優惠券編號'])) {
    echo json_encode(['success' => false, 'message' => '缺少優惠券編號']);
    exit;
}
$rewardId = intval($_POST['優惠券編號']);

try {
    // 查找優惠券的詳細資料（商品名稱和需要點數）
    $stmt = $conn->prepare("SELECT 商品名稱, 需要點數, 優惠券編號 FROM ramen_rewards WHERE 優惠券編號 = ?");
    $stmt->bind_param("i", $rewardId);
    $stmt->execute();
    $reward = $stmt->get_result()->fetch_assoc();

    if (!$reward) {
        echo json_encode(['success' => false, 'message' => '找不到商品']);
        exit;
    }

    // 查詢會員點數
    $stmt = $conn->prepare("SELECT 會員點數 FROM ramen_members WHERE 電話 = ?");
    $stmt->bind_param("s", $phone);
    $stmt->execute();
    $member = $stmt->get_result()->fetch_assoc();
    if (!$member) {
        echo json_encode(['success' => false, 'message' => '找不到會員資料']);
        exit;
    }

    $currentPoints = intval($member['會員點數']);
    $neededPoints = intval($reward['需要點數']);
    if ($currentPoints < $neededPoints) {
        echo json_encode(['success' => false, 'message' => '點數不足，無法兌換']);
        exit;
    }

    // 開始事務處理
    $conn->begin_transaction();

    // 扣除會員點數
    $stmt = $conn->prepare("UPDATE ramen_members SET 會員點數 = 會員點數 - ? WHERE 電話 = ?");
    $stmt->bind_param("is", $neededPoints, $phone);
    $stmt->execute();

    // 新增優惠券，將 `優惠券編號` 儲存到 `ramen_coupons` 中
    $couponName = $reward['商品名稱'];
    $couponId = $reward['優惠券編號'];  // 這裡從 `ramen_rewards` 資料表拿到的優惠券編號
    $receiveTime = date('Y-m-d H:i:s'); 
    $expireDate = date('Y-m-d', strtotime('+3 months'));

    // 在 ramen_coupons 中新增一條優惠券紀錄，並保存對應的優惠券編號
    $stmt = $conn->prepare("INSERT INTO ramen_coupons (優惠券編號, 電話, 優惠券名稱, 狀態, 到期日, 領取時間) VALUES (?, ?, ?, '未使用', ?, ?)");
    $stmt->bind_param("issss", $couponId, $phone, $couponName, $expireDate, $receiveTime);
    $stmt->execute();

    $conn->commit();

    echo json_encode([
        'success' => true,
        'message' => '兌換成功！優惠券已加入您的帳號',
        'coupon' => [
            '優惠券編號' => $couponId,  // 優惠券編號直接使用 `ramen_rewards` 中的編號
            '優惠券名稱' => $couponName,
            '狀態' => '未使用',
            '到期日' => $expireDate,
            '領取時間' => $receiveTime,
            '使用點數' => $neededPoints
        ]
    ]);
    exit;

} catch (Exception $e) {
    $conn->rollback();
    error_log("redeem_reward error: ".$e->getMessage());
    echo json_encode(['success' => false, 'message' => '兌換失敗']);
    exit;
}
?>
